...
1. Open the `MyCo.Identity.iOS\AppDelegate.cs` file.
2. Open the `MyCo.Identity.iOS\info.plist` file in a text editor (opening it in Visual Studio won't work for this step as you need to edit the text)
...
1. Open the `MyCo.Identity.Droid\MsalActivity.cs` file.
...
The `MyCo.Identity.Droid` project requires two extra lines in the `MainActivity.cs` file.
...
The `MyCo.Identity.iOS` project only requires one extra line, in `AppDelegate.cs`.
...
- you might want to right click on the `MyCo.Identity.UWP (Universal Windows)` project and, in the **Application** tab, change the Universal Windows **Target** and **Min Version** depending on the SDK you have installed on your machine.
