...
namespace MyCo.Identity.Client
{
    /// <summary>
    ...
    public partial class MainWindow : Window
    {
        ...
        private static readonly string MyCoIdentityScope = ConfigurationManager.AppSettings["todo:MyCoIdentityScope"];
        private static readonly string MyCoIdentityBaseAddress = ConfigurationManager.AppSettings["todo:MyCoIdentityBaseAddress"];
        private static readonly string[] Scopes = { MyCoIdentityScope };
        private static string MyCoIdentityApiAddress
        {
            get
            {
                string baseAddress = MyCoIdentityBaseAddress;
                return baseAddress.EndsWith("/") ? MyCoIdentityBaseAddress + "api/MyCoIdentity"
                                                 : MyCoIdentityBaseAddress + "/api/MyCoIdentity";
            }
        }
        ...
        private void GetMyCoIdentity()
        {
            GetMyCoIdentity(SignInButton.Content.ToString() != ClearCacheString).ConfigureAwait(false);
        }

        private async Task GetMyCoIdentity(bool isAppStarting)
        {
            ...
            HttpResponseMessage response = await _httpClient.GetAsync(MyCoIdentityApiAddress);
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                Dispatcher.Invoke(() =>
                {
                    MyCoIdentity.ItemsSource = toDoArray.Select(t => new { t.Title });
                });
            }
            else
            {
        ...
        private static async Task DisplayErrorMessage(HttpResponseMessage httpResponse)
        {
            ...
            {
                MessageBox.Show($"{httpResponse.ReasonPhrase}\n {failureDescription}", "An error occurred while getting /api/MyCoIdentity", MessageBoxButton.OK);
            }
        }
        ...
        private async void AddTodoItem(object sender, RoutedEventArgs e)
        {
            ...
            HttpResponseMessage response = await _httpClient.PostAsync(MyCoIdentityApiAddress, content);
        ...
        private async void SignIn(object sender = null, RoutedEventArgs args = null)
        {
            ...
            {
                MyCoIdentity.ItemsSource = string.Empty;
