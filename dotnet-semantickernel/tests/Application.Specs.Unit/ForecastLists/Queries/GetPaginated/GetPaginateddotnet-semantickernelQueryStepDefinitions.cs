...
namespace dotnet_semantickernel.Specs.Application.Unit.ForecastLists.Queries.GetPaginated;

[Binding]
[Scope(Tag = "getPaginateddotnet_semantickernelQuery")]
public class GetPaginateddotnet_semantickernelQueryStepDefinitions : TestBase
{
    private string _def;
    ...
    public void GivenACollectionOfdotnet_semantickernelExist(bool forecastExists)
    {
        _foreacastExists = forecastExists;
    }

    [When(@"I get paginated weather forecasts")]
    public async Task WhenIGetPaginateddotnet_semantickernel()
    {
        var context = new dotnet_semantickernelContext(new DbContextOptionsBuilder<dotnet_semantickernelContext>()
        ...
        if (_validationResponse.IsValid)
            try
            {
                var handler = new Getdotnet_semantickernelPaginatedQueryHandler(context, Mapper);
